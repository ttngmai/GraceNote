  ...
  return (
    <Select.Root value={value} onValueChange={setValue} disabled={disabled}>
      {children ? (
        ...
        <Select.Trigger className="inline-flex items-center justify-center gap-4pxr h-32pxr px-8pxr py-4pxr border border-gray-300 text-[14px] rounded-md shadow-sm hover:bg-[#F8FAFC]">
...
const SelectItem = forwardRef<HTMLDivElement, SelectItemProps>(
  ({ children, value, disabled, showIndicator }, ref) => {
    return (
      <Select.Item
        ref={ref}
        className="flex items-center gap-4pxr min-w-70pxr h-32pxr px-8pxr py-4pxr text-[14px] select-none cursor-pointer hover:bg-[#F8FAFC] hover:font-bold"
        value={value}
        disabled={disabled}
      >
